...
import com.youxin.app.YXApplication;
import com.youxin.app.ui.welcome.WelcomeActivity;
...
public class NimSdkConfig {
    ...
    public static SDKOptions getSDKOptions() {
        ...
        options.sdkStorageRootPath = getAppCacheDir() + "/yx_cache"; // 可以不设置，那么将采用默认路径
    ...
    private static StatusBarNotificationConfig loadStatusBarNotificationConfig() {
        ...
        config.notificationColor = YXApplication.getContext().getResources().getColor(R.color.color_blue_3a9efb);
        // 通知铃声的uri字符串
        ...
        // save cache，留做切换账号备用
        YXCache.setNotificationConfig(config);
        return config;
    }
    ...
    static String getAppCacheDir() {
        ...
        try {
            ...
            if (YXApplication.getContext().getExternalCacheDir() != null) {
                storageRootPath = YXApplication.getContext().getExternalCacheDir().getCanonicalPath();
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
        if (TextUtils.isEmpty(storageRootPath)) {
            ...
            storageRootPath = Environment.getExternalStorageDirectory() + "/" + YXApplication.getContext().getPackageName();
        }
    ...
    private static MixPushConfig buildMixPushConfig() {
        ...
        // 小米推送
        config.xmAppId = "2882303761517862127";
        config.xmAppKey = "5551786239127";
        config.xmCertificateName = "nice";

        // 华为推送 100409139
        config.hwCertificateName = "nicehw";
